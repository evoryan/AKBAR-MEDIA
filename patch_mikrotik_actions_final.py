...
disable_new = """app.post('/api/mikrotik/secrets/:id/disable', async (req, res) => {
    try {
        const { id } = req.params;
        const secretId = req.body.secretId || req.body.secretName; // Use secretId if available
        ...
        // Use raw node-routeros client to execute write
...
enable_new = """app.post('/api/mikrotik/secrets/:id/enable', async (req, res) => {
    try {
        const { id } = req.params;
        const secretId = req.body.secretId || req.body.secretName;
...
remove_new = """app.post('/api/mikrotik/secrets/:id/remove-active', async (req, res) => {
    try {
        ...
        // Find the active connection by name
        const activeRows = await client.rosApi.write('/ppp/active/print', [`?name=${secretName}`]);
