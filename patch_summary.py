...
replacement = """        let totalGlobalRevenue = 0;
        try {
            const [custRows] = await req.pool.query('SELECT price FROM customers WHERE status != "TERHAPUS"');
...
content = re.sub(
    r"res\.json\(\{\s*totalCustomers: customers\[0\]\.total,\s*monthlyRevenue: monthlyRevenue,\s*activePPPoE: 0,\s*activeHotspot: 0,\s*paidCustomers: paidCustomers\[0\]\.paid,\s*unpaidCustomers: unpaidCustomers\[0\]\.unpaid\s*\}\);",
    replacement, content
)
